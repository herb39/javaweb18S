...
<div class="container">
	...
	<div class="text-center">(배송지 정보 확인 후 결제됩니다.)</div>
	<br/>
	<table class="table-bordered text-center" style="margin:auto; width:90%">
	  ...
	  <c:forEach var="vo" items="${sOrderVOS}">  <!-- 세션에 담아놓은 sOrderVOS의 품목내역들을 화면에 각각 보여주는 작업처리 -->
	    <tr align="center">
	      <td><img src="${ctp}/shop/product/${vo.thumbImg}" width="150px"/></td>
	      <td align="left">
	        <p><br/>주문번호 : ${vo.orderIdx}</p>
	        <p class="text-center"><br/>
	          모델명 : <span style="color:orange;font-weight:bold;">${vo.productName}</span><br/>
	          <c:set var="productName" value="${vo.productName}"></c:set>        
		  ...
		  <%-- <input type="hidden" name="cartIdx" value="${idx}"/> --%>  <!-- 장바구니고유번호를 비롯한 주문된 상품의 정보들은 세션에 담겨있기에 굳이 따로 넘길필요없다. 즉 따로 입력된 배송지 정보들만 넘긴다. -->
  ...
  <div style="padding:8px; background-color:#eee;text-align:center;">
    총 주문(결제) 금액 : 상품가격(<fmt:formatNumber value="${orderTotalPrice}" pattern='#,###원'/>) +
                    배송비(<fmt:formatNumber value="${sOrderVOS[0].delivery}" pattern='#,###원'/>) =
                    총 <font size="4" color="orange"><b><fmt:formatNumber value="${orderTotalPrice + sOrderVOS[0].delivery}" pattern='#,###'/></b></font>원
  </div>
  </td></tr></table>
	<p><br/></p>
	<form name="myform" method="post">
	  <table class="table table-bordered text-center">
		    ...
		    <tr>
			  <th>배송지 주소</th>
			  <td class="text-left">
			    <input type="text" name="buyer_postcode" value="" class="form-control" /><br/>
			    <input type="text" name="buyer_addr" value="" class="form-control"/>
			  </td>
			</tr>
		    <tr>
			  <th>배송시 요청사항</th>
			  <td><!-- <input type="text" name="message" class="form-control" placeholder="배송시요청사항을 기록하세요"/> -->
			    <select name="message" class="form-control">
			      <option>직접수령</option>
			      <option>부재시 경비실</option>
			      <option>부재시 현관문 앞</option>
			      <option>부재시 반송</option>
			    </select>
			  </td>
			</tr>
	    	<tr>
			  <th>총 결제금액(테스트 10원)</th>
	  ...
	  <div class="tab-content">
		...
		<div align="center">
		  <button type="button" class="btn btn-dark" onClick="order()">결제하기</button> &nbsp;
			<button type="button" class="btn btn-dark" onclick="location.href='${ctp}/shop/cartList';">장바구니보기</button> &nbsp;
		  <button type="button" class="btn btn-dark" onClick="location.href='${ctp}/shop/productList';">계속 쇼핑하기</button>
		</div>
	  ...
	  <input type="hidden" name="name" value="${productName}"/>	<!-- 결제창으로 넘겨줄 모델명 -->
	  <%-- <input type="hidden" name="amount" value="${orderTotalPrice}"/> --%>	<!-- 결제창으로 넘겨줄 결제금액 -->
	</form>
</div>
<p><br/></p>
