...
<head>
  ...
  <script>
    ...
    function nWin(orderIdx) {
    	...
    	window.open(url,"orderDelivery","width=390px,height=430px");
    }
    ...
    function myOrderStatus() {
    	...
    	location.href="${ctp}/shop/myOrderStatus?startJumun="+startJumun+"&endJumun="+endJumun+"&conditionOrderStatus="+conditionOrderStatus+"&pag=${pageVO.pag}";
    }
  </script>
</head>
<body>
...
<div class="container">
  ...
  <div class="text-center" style="font-size: 28px;">주문 / 배송 조회</div><br />
  <hr/>
  <table class="table table-borderless">
    <tr>
      <td style="text-align:left;">주문조회 : 
        ...
        <input type="button" class="btn" value="오늘" onclick="orderCondition(1)"/>
        <input type="button" class="btn" value="일주일이내" onclick="orderCondition(7)"/>
        <input type="button" class="btn" value="15일이내" onclick="orderCondition(15)"/>
        <input type="button" class="btn" value="30일이내" onclick="orderCondition(30)"/>
        <input type="button" class="btn" value="90일이내" onclick="orderCondition(90)"/>
        <input type="button" class="btn" value="전체조회" onclick="orderCondition(99999)"/>
      </td>
    ...
    <tr>
      ...
      <td style="text-align:right;">
	      <a href="${ctp}/shop/cartList" class="btn btn-success btn-sm">장바구니조회</a>
    ...
    <c:forEach var="vo" items="${vos}">
      <tr>
        <td style="text-align:center;">
          ...
          <p>총 주문액 : <fmt:formatNumber value="${vo.totalPrice}"/>원</p>
          <p><input type="button" value="배송지정보" class="btn" onclick="nWin('${vo.orderIdx}')"></p>
        </td>
  ...
  <!-- 블록 페이징처리 시작(BS4 스타일적용) -->
		...
		<ul class="pagination justify-content-center">
			...
			<c:if test="${pageVO.totPage != 0}">
			  <c:if test="${pageVO.pag != 1}">
			    <li class="page-item"><a href="${ctp}/shop/myOrder?pag=1" title="첫페이지" class="page-link text-secondary">◁◁</a></li>
			  </c:if>
			  <c:if test="${pageVO.curBlock > 0}">
			    <li class="page-item"><a href="${ctp}/shop/myOrder?pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}" title="이전블록" class="page-link text-secondary">◀</a></li>
			  </c:if>
			  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize)+pageVO.blockSize}">
			    <c:if test="${i == pageVO.pag && i <= pageVO.totPage}">
			      <li class="page-item active"><a href='${ctp}/shop/myOrder?pag=${i}' class="page-link text-light bg-secondary border-secondary">${i}</a></li>
			    </c:if>
			    <c:if test="${i != pageVO.pag && i <= pageVO.totPage}">
			      <li class="page-item"><a href='${ctp}/shop/myOrder?pag=${i}' class="page-link text-secondary">${i}</a></li>
			    </c:if>
			  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}">
			    <li class="page-item"><a href="${ctp}/shop/myOrder?pag=${(pageVO.curBlock+1)*pageVO.blockSize + 1}" title="다음블록" class="page-link text-secondary">▶</a>
			  </c:if>
			  <c:if test="${pageVO.pag != pageVO.totPage}">
			    <li class="page-item"><a href="${ctp}/shop/myOrder?pag=${pageVO.totPage}" title="마지막페이지" class="page-link" style="color:#555">▷▷</a>
			  </c:if>
			</c:if>
		</ul>
	</div>
	<!-- 블록 페이징처리 끝 -->
  <hr/>
  <p class="text-center"><a href="${ctp}/shop/shopList" class="btn btn-primary">계속쇼핑하기</a></p>
  <hr/>
<p><br/></p>
</div>
