...
<div class="container">
  ...
  <c:if test="${conditionDate=='15'}"><c:set var="condition" value="15일 이내 조회"/></c:if>
  <c:if test="${conditionDate=='30'}"><c:set var="condition" value="30일 이내 조회"/></c:if>
  <c:if test="${conditionDate=='90'}"><c:set var="condition" value="90일 이내 조회"/></c:if>
    ...
    <tr>
      <td style="text-align:left;">주문조회 : 
			  <c:choose>
			    ...
			    <c:when test="${conditionDate == '15'}"><c:set var="conditionDate" value="15일이내"/></c:when>
			    <c:when test="${conditionDate == '30'}"><c:set var="conditionDate" value="30일이내"/></c:when>
			    <c:when test="${conditionDate == '90'}"><c:set var="conditionDate" value="90일이내"/></c:when>
			    <c:otherwise><c:set var="conditionDate" value="전체"/></c:otherwise>
			  </c:choose>
        <input type="button" class="btn btn-light" value="오늘" onclick="orderCondition(1)"/>
        <input type="button" class="btn btn-light" value="일주일이내" onclick="orderCondition(7)"/>
        <input type="button" class="btn btn-light" value="15일이내" onclick="orderCondition(15)"/>
        <input type="button" class="btn btn-light" value="30일이내" onclick="orderCondition(30)"/>
        <input type="button" class="btn btn-light" value="90일이내" onclick="orderCondition(90)"/>
        <input type="button" class="btn btn-light" value="전체조회" onclick="orderCondition(99999)"/>
      </td>
      <td></td>
    </tr>
    <c:if test="${sLevel == 0}">
	    <tr>
	      <td style="text-align:left;">날짜기간 및 조건검색 :
	        ...
	        <input type="button" class="btn btn-light" value="조회하기" onclick="myOrderStatus()"/>
	      </td>
	      <td style="text-align:right;">
		      <a href="${ctp}/shop/cartList" class="btn btn-dark btn-sm">장바구니조회</a>
		      <a href="${ctp}/shop/productList" class="btn btn-dark btn-sm">계속쇼핑하기</a>
	      </td>
	    </tr>
    </c:if>
  </table>
    ...
    <c:forEach var="vo" items="${vos}">
      <tr>
        <td style="text-align:center;">
          ...
          <p><input type="button" value="배송지 정보" class="btn btn-light" onclick="nWin('${vo.orderIdx}')"></p>
        </td>
        <td style="text-align:center;"><br/><img src="${ctp}/data/shop/product/${vo.thumbImg}" class="thumb" width="100px"/></td>
        <td align="left">
  ...
  <!-- 블록 페이징처리 시작(BS4 스타일적용) -->
		...
		<ul class="pagination justify-content-center">
			...
			<c:if test="${pageVO.totPage != 0}">
			  <c:if test="${pageVO.pag != 1}">
			    <li class="page-item"><a href="${ctp}/shop/myOrderStatus?pag=1&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}" title="첫페이지" class="page-link text-secondary">◁◁</a></li>
			  </c:if>
			  <c:if test="${pageVO.curBlock > 0}">
			    <li class="page-item"><a href="${ctp}/shop/myOrderStatus?pag=${(pageVO.curBlock-1)*pageVO.blockSize + 1}&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}" title="이전블록" class="page-link text-secondary">◀</a></li>
			  </c:if>
			  <c:forEach var="i" begin="${(pageVO.curBlock*pageVO.blockSize)+1}" end="${(pageVO.curBlock*pageVO.blockSize)+pageVO.blockSize}">
			    <c:if test="${i == pageVO.pag && i <= pageVO.totPage}">
			      <li class="page-item active"><a href='${ctp}/shop/myOrderStatus?pag=${i}&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}' class="page-link text-light bg-secondary border-secondary">${i}</a></li>
			    </c:if>
			    <c:if test="${i != pageVO.pag && i <= pageVO.totPage}">
			      <li class="page-item"><a href='${ctp}/shop/myOrderStatus?pag=${i}&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}' class="page-link text-secondary">${i}</a></li>
			    </c:if>
			  </c:forEach>
			  <c:if test="${pageVO.curBlock < pageVO.lastBlock}">
			    <li class="page-item"><a href="${ctp}/shop/myOrderStatus?pag=${(pageVO.curBlock+1)*pageVO.blockSize + 1}&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}" title="다음블록" class="page-link text-secondary">▶</a>
			  </c:if>
			  <c:if test="${pageVO.pag != pageVO.totPage}">
			    <li class="page-item"><a href="${ctp}/shop/myOrderStatus?pag=${pageVO.totPage}&startJumun=${startJumun}&endJumun=${endJumun}&conditionOrderStatus=${conditionOrderStatus}" title="마지막페이지" class="page-link" style="color:#555">▷▷</a>
			  </c:if>
			</c:if>
		</ul>
	</div>
	<!-- 블록 페이징처리 끝 -->
  <hr/>
  <p class="text-center"><a href="${ctp}/shop/productList" class="btn btn-dark">계속쇼핑하기</a></p>
  <hr/>
<p><br/></p>
</div>
